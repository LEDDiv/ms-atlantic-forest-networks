...
# occ
all_occ <- read_csv("01_data/00_occurrences/00_flora/flora_occ.csv")
all_occ
head(all_occ)

da_occ <- count(all_occ, species, sort = TRUE)
da_occ %>% filter(n >= 10) # ref. relacionada a plantas e animais na Mata Atlântica

da_int <- count(readr::read_csv2("../../Master/BIOMAS/Andre/matrizes_andre_dados1cap/planilhas biomas_cap1/Mata_atlantica_int.csv"), plant, sort = TRUE)

da_occ_int <- dplyr::left_join(da_occ, da_int, by = join_by(species == plant))
...
li <- sf::st_read("../../coisas mestrado_espacial/america.shp")
li
...
#covar
files <- dir(path = "01_data/01_variables/02_adjusted/", pattern = ".tif", full.names = T )
covar <- terra::rast(files)
names(covar) <- c("bio01", paste0("bio", 10:19), paste0("bio0", 2:9), "bdod", 
                  "slope_md", "slope_sd", "cfvo", "clay", "phh2o", "sand", "silt")
names(covar)
...
for(i in unique(occ_sa$species[100682:101247])){ 
  ...
  if(n_occ < 10){
    ...
    path_sp <- paste0("02_results/00_flora/", i)
    ...
    ## map
    dir.create("02_results/00_flora/00_maps")
    map_enm <- tm_shape(eval_fit_aic_predict) +
      ...
      tm_grid(lines = FALSE) +
      tm_title(i)
    tmap::tmap_save(map_enm, paste0("02_results/00_flora/00_maps/map_enm_", i, ".png"), 
    ...
    map_enm_af <- tm_shape(eval_fit_aic_predict_af) +
      ...
      tm_grid(lines = FALSE) +
      tm_title(i)
    tmap::tmap_save(map_enm_af, paste0("02_results/00_flora/00_maps/map_enm_", i, "_af.png"), 
                    wi = 20, he = 18, un = "cm", dpi = 300)
    ...
    map_enm_af_msdm <- tm_shape(eval_fit_aic_predict_af_msdm) +
      ...
      tm_grid(lines = FALSE) +
      tm_title(i)
    tmap::tmap_save(map_enm_af_msdm, paste0("02_results/00_flora/00_maps/map_enm_", i, "_af_msdm.png"), 
                    wi = 20, he = 18, un = "cm", dpi = 300)
