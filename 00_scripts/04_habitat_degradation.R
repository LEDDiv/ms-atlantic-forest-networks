...
## fire ----
fire_files <- dir(path = "01_data/01_variables/04_degradation/raw", pattern = "fire", full.names = TRUE)
fire_files
...
af <- terra::vect("limites/MA_ecorr.shp") %>% 
  terra::crop(br)
...
## grid ----
grid_r <- terra::rast("01_data/01_variables/04_degradation/veg.tif") %>% 
  terra::mask(af)
grid_r
plot(grid_r)
...
## import rasters ----
habitat <- terra::rast("01_data/01_variables/04_degradation/habitat.tif") %>% 
  terra::mask(af)
habitat
plot(habitat)
...
# export
terra::writeRaster(grid_habitat_loss_raster, "01_data/01_variables/04_degradation02_results/degradation/habitat_loss.tif", overwrite = TRUE)
terra::writeRaster(grid_distance_raster, "01_data/01_variables/04_degradation02_results/degradation/distance_raster.tif", overwrite = TRUE)
terra::writeRaster(grid_edge90m_raster, "01_data/01_variables/04_degradation02_results/degradation/edge90m_raster.tif", overwrite = TRUE)
terra::writeRaster(grid_fire_raster, "01_data/01_variables/04_degradation02_results/degradation/fire_raster.tif", overwrite = TRUE)
terra::writeRaster(grid_secondary_raster, "01_data/01_variables/04_degradation02_results/degradation/secondary_raster.tif", overwrite = TRUE)
terra::writeRaster(grid_degradation_index_raster, "01_data/01_variables/04_degradation02_results/degradation/degradation_index_raster.tif", overwrite = TRUE)

# end ---------------------------------------------------------------------

grid_degradation_index_raster_af <- terra::rast("02_results/degradation/degradation_index_raster.tif") %>% 
  terra::mask(af)
grid_degradation_index_raster_af

terra::writeRaster(grid_degradation_index_raster_af, "02_results/degradation/degradation_index_raster_af.tif", overwrite = TRUE)
